...
Throughout the weeks, I attempted to iterate on the visualization to see what patterns could be discovered. While initially I wanted to do a geospatial viz, it turned out to be far more interesting to compare the multitude of available attributes in an exploratory system.

I started by trying to answer the question of "How does the price of a unit compare across its capacity?"
After visualizing this, there is a clear upward trend in the average price as the person's capacity increases. However, the maximum price for a 2/3 person unit is higher than a 4/5/6 person unit. This could be a due to "extreme luxury" rentals meant for couples' getaways.
I explored how distributions can be more effectively displayed. After some trial and error, the solution I came up with was to use a "scatterplot-like" system where each candlestick bar on the chart is not just a box, but a blob of points made up of each datapoint. This allows you to see the min/avg/max as before, but now the more detailed distribution of each candlestick on the plot.
...
I then added colors as a way to visualize the data. Previously, the chart showed the density of its listings by how "packed" the bar was at that point, but I was not happy with that, as it was not a very accurate way to see the data, though it looked nice and conveyed other helpful information. This week, I switched the scatterplot style bars to be heatmap style bars, which show the density of listings across the Y axis. This immediately changes what the data shows, which is that the vast majority of listings fall around the cheaper 2-person type. This was not shown well previously. I ultimately chose to use a blue-to-yellow gradient to represent the density gradient for accessibility reasons, as blue and yellow remain contrasting in normal vision and in the major variants of colorblindness.
I also added interactivity to the viz in the form of selectors and a slider that allows the user to dynamically manipulate what is shown on the graph to make it easier to garner insignts from the chart.
...
I then added more interactivity support, allowing for selecting multiple cities at once and having their data combined into 1 big viz. For polishing, I added a display map for the axis/filter names (not just raw column names), and put a title on the viz that dynamically updates to reflect what is being shown. As well as a tooltip that shows the exact value the user is hovering over on the chat, and a sidebar with explanations for what each selectable attribute means in the context of Airbnb rentals.
...
For the final iteration, I added per-column density calculations, logarithmic density scaling, and converted it to grayscale for clarity.
